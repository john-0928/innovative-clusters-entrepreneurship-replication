********************************************************************************
* Replication package: innovative industrial cluster pilots and entrepreneurial vitality
* Run this file from the package root directory.
* The exact Stata release used for the submitted results was not supplied.
* Before publication, add a version statement matching the original environment.
********************************************************************************
...
global DATA_DIR "${PROJECT_ROOT}/data/stata"
global OUTPUT_DIR "${PROJECT_ROOT}/outputs"
global TABLE_DIR "${OUTPUT_DIR}/tables"
global FIGURE_DIR "${OUTPUT_DIR}/figures"
global DERIVED_DIR "${OUTPUT_DIR}/derived_data"
global DESCRIPTIVE_DIR "${OUTPUT_DIR}/descriptive"
...
log using "${LOG_DIR}/master_analysis.log", text replace name(master)

***Calculate the changes in entrepreneurial vitality between pilot and non pilot cities of innovative industrial clusters
clear all
set more off
use "${DATA_DIR}/master_data.dta",clear
collapse (mean) entreact, by(year treat)
rename entreact ent_mean
sort treat year
export delimited using "${DESCRIPTIVE_DIR}/entrepreneurial_vitality_by_treatment.csv", replace
...
esttab n1 n2 n3 using "${TABLE_DIR}/Benchmark regression results.rtf",  b(%6.3f) ci(%6.3f)  star( * 0.1 ** 0.05 *** 0.01) replace nogap  scalars(N r2_a) sfmt(%20.0f %6.3f )	
...
esttab n1  using "${TABLE_DIR}/Parallel trend test.rtf",  b(%6.3f) ci(%6.3f) star( * 0.1 ** 0.05 *** 0.01) replace nogap  scalars(N r2_a) sfmt(%20.0f %6.3f )	
...
esttab se_robust se_city se_year se_cityyear using "${TABLE_DIR}/Sensitivity analysis.rtf",  b(%6.3f) ci(%6.3f) star( * 0.1 ** 0.05 *** 0.01) replace nogap  scalars(N r2_a) sfmt(%20.0f %6.3f )	
...
forvalues i = 1/1000{ 
 use "${DATA_DIR}/placebo_test.dta", clear
 xtset city year
 keep if year==2009
 sample 78, count 
 keep city 
 save "${DERIVED_DIR}/match_city.dta", replace 
 merge 1:m city using "${DATA_DIR}/placebo_test.dta"
 gen ntreat=(_merge==3)  
 ...
 save "${DERIVED_DIR}/placebo_simulation.dta", replace 
...
graph export "${FIGURE_DIR}/placebo_test.png", replace width(2400)
 
 	   
**PSM-DID inspection
* Reload the analysis panel because the placebo block leaves only simulation output in memory.
use "${DATA_DIR}/master_data.dta", clear
global var gdp fin stru wage gov human pubservice
...
esttab  n1 n2 n3 using "${TABLE_DIR}/PSM-DID inspection.rtf", b(%6.3f) ci(%6.3f) star( * 0.1 ** 0.05 *** 0.01) replace nogap  scalars(N r2_a) sfmt(%20.0f %6.3f )	 
...
esttab n1 n2 n3 n4 using "${TABLE_DIR}/Estimation results of instrumental variable method.rtf", replace b(%6.3f) ci(%6.3f) star(* 0.10 ** 0.05 *** 0.01) nogap mtitles("(1) Phase One" "(2) Phase Two" "(3) Phase One" "(4) Phase Two") mlabels("IV=iv_hl" "IV=iv_hl" "IV=iv_sy" "IV=iv_sy") scalars(N r2) sfmt(%12.0f)
...
esttab n1 n2 n3 n4 n5 n6 n7 n8 using "${TABLE_DIR}/Other robustness tests.rtf", b(%6.3f) ci(%6.3f) star( * 0.1 ** 0.05 *** 0.01) replace nogap  scalars(N r2_a) sfmt(%20.0f %6.3f )
...
esttab simpleatt groupatt timeatt eventatt using "${TABLE_DIR}/Heterogeneity robust estimator test (CSDID).rtf",replace b(%6.3f) ci(%6.3f) star(* 0.10 ** 0.05 *** 0.01) mtitles("Simple ATT" "Group ATT" "Calendar ATT" "Event ATT")
...
esttab n1 n2 n3 n4 n5 n6 using "${TABLE_DIR}/Mechanism testing regression results.rtf", b(%6.3f) ci(%6.3f) star( * 0.1 ** 0.05 *** 0.01) replace nogap  scalars(N r2_a) sfmt(%20.0f %6.3f )
...
esttab n1 n2 n3 n4  using "${TABLE_DIR}/Analysis results of heterogeneity of urban characteristics.rtf", b(%6.3f) ci(%6.3f) star( * 0.1 ** 0.05 *** 0.01) replace nogap  scalars(N r2_a) sfmt(%20.0f %6.3f )
...
esttab n1 n2 n3 n4 using "${TABLE_DIR}/Analysis results of market environment heterogeneity.rtf", b(%6.3f) ci(%6.3f) star( * 0.1 ** 0.05 *** 0.01) replace nogap  scalars(N r2_a) sfmt(%20.0f %6.3f )
...
esttab  n1 n2 n3 n4 using "${TABLE_DIR}/Analysis results of heterogeneity in industry distribution.rtf", b(%6.3f) ci(%6.3f) star( * 0.1 ** 0.05 *** 0.01) replace nogap  scalars(N r2_a) sfmt(%20.0f %6.3f )
...
**adjacency matrix
cd "${PROJECT_ROOT}"
use "${DATA_DIR}/spatial_adjacency_source.dta", clear
...
clear
svmat w11, names(col)
save "${DERIVED_DIR}/spatial_adjacency_weights.dta", replace
restore
...
cd "${PROJECT_ROOT}"
use "${DATA_DIR}/spatial_distance_source.dta",clear
...
clear
svmat w22, names(col)
save "${DERIVED_DIR}/spatial_distance_weights.dta", replace
restore
...
cd "${PROJECT_ROOT}"
use "${DATA_DIR}/spatial_geo_economic_source.dta",clear
...
clear
svmat w33, names(col)
save "${DERIVED_DIR}/spatial_geo_economic_weights.dta", replace
restore
...
cd "${PROJECT_ROOT}"
use "${DATA_DIR}/spatial_effect_master_data.dta",clear

spatwmat using "${DERIVED_DIR}/spatial_adjacency_weights.dta", n(w11) standardize
matrix list w11
...
save "${DERIVED_DIR}/moran_results_adjacency.dta", replace

**Global Moran Index (Geographic Distance Matrix)
cd "${PROJECT_ROOT}"
use "${DATA_DIR}/spatial_effect_master_data.dta",clear

spatwmat using "${DERIVED_DIR}/spatial_distance_weights.dta", n(w22) standardize
matrix list w22
...
save "${DERIVED_DIR}/moran_results_distance.dta", replace
...
spatwmat using "${DERIVED_DIR}/spatial_geo_economic_weights.dta", n(w33) standardize
matrix list w33
...
save "${DERIVED_DIR}/moran_results_geo_economic.dta", replace
...
esttab sdm_w11 sdm_w22 sdm_w33 using "${TABLE_DIR}/Results of Spatial Spillover Effect Test.rtf", replace  b(%6.3f) ci(%6.3f)  star(* 0.10 ** 0.05 *** 0.01)  keep(did Wx:did LR_Direct:did LR_Indirect:did LR_Total:did) nogap scalars(N r2) sfmt(%20.0f %6.3f ) mtitles("w11-SDM" "w22-SDM" "w33-SDM")
...
use "${DATA_DIR}/traffic_distance_data.dta", clear
tempfile distbase
save `distbase'
...
esttab m using "${TABLE_DIR}/Results of Spatial Overflow Boundary Inspection.rtf", replace  b(%6.3f) ci(%6.3f)  star(* 0.10 ** 0.05 *** 0.01) nogap scalars(N r2) sfmt(%20.0f %6.3f )
